...
class PlaybackViewController: UIViewController {
    ...
    private func createCompleteBoneStructure() {
        // Define bone connections for available ARSkeleton.JointName values
        let boneConnections: [(ARSkeleton.JointName, ARSkeleton.JointName)] = [
            // Basic skeleton structure with available joint names
            (.root, .leftShoulder),
            (.root, .rightShoulder),
            (.root, .head),
    ...
    private func getBoneWidth(from startJoint: ARSkeleton.JointName, to endJoint: ARSkeleton.JointName) -> Float {
        // Make all bones a consistent width for simplicity
        if startJoint == .root || endJoint == .root {
            return 0.03 // Slightly thicker for main body connections
        } else {
            return 0.02 // Standard bone width
        }
    }
    ...
    private func updateSkeletonPose(frame: RecordedFrame) {
        ...
        for boneEntity in boneEntities {
            let boneName = boneEntity.name ?? ""
            let jointNames = boneName.components(separatedBy: "-")
            guard jointNames.count == 2 else { continue }
            
            // Find the joint names that match our recorded joints
            var startJointName: ARSkeleton.JointName?
            var endJointName: ARSkeleton.JointName?
            
            // Match against our available joints
            let availableJoints: [ARSkeleton.JointName] = [.root, .leftHand, .rightHand, .leftShoulder, .rightShoulder, .leftFoot, .rightFoot, .head]
            
            for joint in availableJoints {
                if joint.rawValue == jointNames[0] {
                    startJointName = joint
                }
                if joint.rawValue == jointNames[1] {
                    endJointName = joint
                }
            }
            
            guard let startJoint = startJointName,
                  let endJoint = endJointName else {
                continue
            }
            
            guard let startTransform = frame.jointTransforms[startJoint],
                  let endTransform = frame.jointTransforms[endJoint] else {
                continue
            }
